                        ...
                        <div class="col-lg-6 col-xl-6 col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="current-balance-outer bg">
                                                    ...
                                                    <div class="coin-block-outer slide-track">
                                                        <div class="coin-block slide">
                                                            ...
                                                            <div class="coin-block-inner">
                                                                <div><img
                                                                        src="{{ url('/img/color/' . $coinlist->image) }}">
                                                                </div>
                                ...
                                <div class="showall-btn">
                                    <a href="{{ route('wallet') }}">Go to Wallet <img
                                            src="{{ url('img/arrow-right.svg') }}" alt="arrow" /></a>
                                </div>
...
@else
<a href="{{ route('kycform') }}" class="btn btn-primary error-message"><i class="fa fa-times-circle text-danger"></i> Your KYC and KYB is not verified. </a>
...
@endif
                                ...
                                <div class="security backcolor dash">
                                    ...
                                    <div class="enable-btn">
                                        @if (Auth::user()->role == 'Business')
                                            <button class="btn btn-primary success-message"><i
                                                    class="fa fa-check-circle text-success"></i> Your KYB is
                                                verified</button>
                                        @else
                                            <button class="btn btn-primary success-message"><i
                                                    class="fa fa-check-circle text-success"></i> Your KYC is
                                                verified</button>
                                        @endif
                                    </div>
                                </div>
                            @else
                                <div class="security backcolor dash error">
                                    <div class="enable-btn">
                                        @if (Auth::user()->role == 'Business')
                                            <a class="btn btn-primary error-message"><i
                                                    class="fa fa-times-circle text-danger"></i> Your KYB is not
                                                verified. </a>
                                        @else
                                            <a class="btn btn-primary error-message"><i
                                                    class="fa fa-times-circle text-danger"></i> Your KYC is not
                                                verified. </a>
                                        @endif
                                        <a href="{{ route('security') }}" class="error-verify">Verify Now <img
                                                src="{{ url('img/arrow-right.svg') }}" alt="arrow" /></a>
                                    </div>
                                ...
                                <div class="current-balance-bottom">
                                    @forelse($comdetails as $key => $coinlist)
                                        <div class="current-balance-bottom-inner">
                                            <div class="coin-coin">
                                                <span class="coin-coin-1"><img
                                                        src="{{ url('img/color/' . $coinlist->image) }}" />
                                                    {{ $coinlist->coinname }}</span>
                                                <span class="coin-coin-2">$ <span
                                                        class="last_price_{{ $coinlist->source }}USD">{{ $coinlist->usd_price }}</span></span>
                                                <span class="coin-coin-3">(<span
                                                        class="price_change_{{ $coinlist->source }}USD">-2.48%</span>)</span>
                                            </div>
                                        </div>
                                    @empty
                                    @endforelse
                                        ...
                                        <tbody>
                                            @if (count($history) > 0)
                                                ...
                                                @foreach ($history as $transaction)
                                                    <tr>
                                                        <td>{{ $i }}</td>
                                                        <td>{{ date('d-m-Y H:i:s', strtotime($transaction->created_at)) }}
                                                        </td>
                                                        <td>{{ 'From' }}</td>
                                                        @if (isset($transaction->BuyerInformation['first_name']))
                                                            <td>{{ $transaction->BuyerInformation['first_name'] }}
                                                                {{ $transaction->BuyerInformation['last_name'] }}</td>
                                                        @else
                                                            <td>POS Deposit</td>
                                                        @endif
                                                        ...
                                                        <td>
                                                            @if ($transaction->status == 0)
                                                                <a
                                                                    href="{{ url('/trans_view/' . $transaction->txn_id) }}">Waiting</a>
                                                            @elseif($transaction->status == -1)
                                                                <a
                                                                    href="{{ url('/trans_view/' . $transaction->txn_id) }}">
                                                                    Cancelled </a>
                                                            @elseif($transaction->status == 1)
                                                                <a
                                                                    href="{{ url('/trans_view/' . $transaction->txn_id) }}">Confirmed</a>
                                                            @elseif($transaction->status == 2)
                                                                <a
                                                                    href="{{ url('/trans_view/' . $transaction->txn_id) }}">Queued</a>
                                                            @elseif($transaction->status == 100)
                                                                <a
                                                                    href="{{ url('/trans_view/' . $transaction->txn_id) }}">Completed</a>
                                                            @endif
